...
def OCT(X, y, D=3, alpha=1e-7, Nmin=5, timelimit=None, warmStart=False, verbose=False):
...
def OCTH(X, y, D=3, alpha=1e-7, Nmin=5, timelimit=None, verbose=True):
    # ---- Pre-processing steps ---- #
    K = len(np.unique(y))  # number of classes
    N = len(y)  #  number of observations
    p = X.shape[1]
...
class OptimalTreeClassifier:
    ...
    def __init__(self, D=3, multivariate=False, alpha=0.01, Nmin=5):
    ...
    def predict(self, X):
        ...
        for i in range(n):
            ...
            pred_y[i] = np.argwhere(self.C[leaf, :] == 1)[0, 0]
...
if __name__ == '__main__':
    ...
    OCTclassifier = OptimalTreeClassifier(D=3, alpha=0.1, multivariate=False)
    OCTclassifier.train(Xtrain,Ytrain, timelimit=1500, verbose=True)
